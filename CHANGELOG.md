...
- **`cc-plugin-eval` plugin (0.1.0 → 0.2.0)**: New **CC710** check (#304) flags subagent references that resolve to nothing — an agent/skill that names a teammate which no manifest provides. #308 made the CC710 directory walk a portable recursive `readdir` and synced `package.json` to 0.2.0.

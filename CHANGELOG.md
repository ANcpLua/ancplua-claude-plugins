...
- **`claudemd-curator` plugin (0.1.0)**: Forked from `anthropics/claude-plugins-official` `claude-md-management` (Isabella He, Anthropic). Phase 1 modifications: discovery glob expanded beyond `CLAUDE.md` to also surface `AGENTS.md` (Codex / OpenAI / Codeium standard, peer of CLAUDE.md), `.claude/rules/*.md` (auto-loaded rule files), and recognise `~/.claude/projects/*/memory/MEMORY.md` as read-only awareness (auto-memory subsystem owns the format). New `update-guidelines.md` section calling out the CLAUDE.md ↔ AGENTS.md split: the same fact often must live in both files because each tool reads only its own. `/revise-claudemd` command (renamed from `/revise-claude-md`) presents all four targets for end-of-session capture. Original Anthropic `LICENSE` retained inside the plugin. Phase 2 (workspace-wide multi-repo scan, plugin-aware audit cross-referenced with `marketplace-tour` capability snapshot) tracked in `AGENTS.md` "Next session — start here".
- **`session-debrief` plugin (0.1.0)**: Forked from `anthropics/claude-plugins-official` `session-report`. Phase 1 modifications: renamed plugin and skill (debrief = after-action review, matching what the HTML actually delivers; avoids collision with Claude Code's first-party "recap" feature). Added `.claude-plugin/plugin.json` (upstream shipped without one — the marketplace requires it). Added `README.md` (upstream shipped without one). Output filename pattern `session-report-` → `session-debrief-`. Bundled assets (875-line streaming transcript parser `analyze-sessions.mjs` + 569-line self-contained `template.html`) preserved byte-identical for now. Original Anthropic `LICENSE` retained inside the plugin. Phase 2 (cache-rot >300k cliff flag, effort-level attribution from `~/.claude/settings.json`, marketplace-aware project tier mapping, default output `~/.claude/reports/`, smoke-test harness) tracked in `AGENTS.md` "Next session — start here".
...
- **`claudemd-curator` / `session-debrief` PR review cleanup**: Aligned command `allowed-tools` with required shell/write operations, made project-memory reports cover all discovered artifacts instead of only `CLAUDE.md`, fixed nested markdown fences in curator templates, corrected Apache-2.0 plugin manifests, removed external Google Fonts from the self-contained debrief template, documented safe JSON embedding for `report-data`, preserved prompt/subagent attribution before `--since` while gating only accounting, and synced README / marketplace / Copilot coordination text with the new plugins.
- **`mutation-minded-testing` plugin manifest**: Changed `agents` from a directory string to explicit agent file paths so `claude plugin validate` accepts the plugin manifest.

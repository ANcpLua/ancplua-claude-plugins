...
- **`heimdall` plugin (0.1.0 → 0.2.0)**: Model-routing watchman. Answers "which model actually served me?" from evidence — the session transcript's `message.model`, stamped by the inference server — not from latency, config, or a UI banner. `/heimdall` reports served model per turn, per-model token usage, the current model, and any mid-session Opus↔Fable fallback swaps; `scripts/which-provider.sh` adds provider/region/auth/edge routing context (honestly scoped: it locates the user, not the model). A passive, non-blocking `model-drift-net` Stop hook surfaces a one-line notice the moment the served model changes and stays silent otherwise. **v0.2.0** adds `scripts/session-inspect.sh`, the extended transcript read: `advisorModel`, `service_tier`, `speed`, `inference_geo`, cc version/entrypoint, token totals, cache **hit rate**, and cache-**miss reasons** with re-charged token counts + `stop_reason` distribution — turning the transcript into per-session cost/routing observability (e.g. surfacing millions of tokens re-charged by `messages_changed`/`tools_changed` cache busts). Deterministic, no-LLM, no-network, no secrets.

- **`charon` plugin (0.1.0 → 0.3.0)**: PR-to-merge ferry that never waits forever. Started as a full-vision v0.1.0 (#293) — a clock-independent `Stop` hook that ferries a GitHub PR to merge, proposes-and-pauses on force operations, and keeps a GitHub-only seam with a dispatch-table extension model — then collapsed to one arg-dispatched command (#294). v0.2.0 made it self-bootstrapping (#298): "merge my PR" works with no slash command. v0.2.1 (#309) honors an explicitly named PR over stale local state and skips solo-bootstrap for fleet lookouts. v0.3.0 removes the `reviewer-triage` skill — reviewer feedback is handled inline by the `review-changes` handler in one pass, no sub-agents, no cross-plugin skill invocations (the skill text was gifted to a sibling skills repo).
...
- **`cc-plugin-eval` plugin (0.1.0 → 0.2.1)**: New **CC710** check (#304) flags subagent references that resolve to nothing — an agent/skill that names a teammate which no manifest provides. #308 made the CC710 directory walk a portable recursive `readdir` and synced `package.json` to 0.2.0. v0.2.1 fixes the frontmatter parser: multi-line quoted scalars (backslash-escaped breaks, `\uXXXX`/escaped-space sequences — the exact shape the since-removed cross-CLI marketplace migration generated) parsed as "Unexpected indentation" and produced false CC201/CC205/CC213 failures; quoted scalars are now folded and unescaped per YAML 1.2.
...
- **Cross-CLI purge (repo-wide)**: the marketplace is Claude Code only — no other CLI mirrors, no non-Claude models. Deleted the second-CLI marketplace mirror (`.agents/plugins/marketplace.json`), all 13 second-CLI plugin manifests, all 24 auto-generated `source-command-*` migration wrapper skills (which Claude Code was loading as real skills and surfacing in installed components), and the third-party review prompt + output schema under `.github/`. Docs updated to match: CLAUDE.md (+ `.cursor`/`.windsurf` mirrors) drop the dual-manifest rule, README drops the second-CLI install section and the stale auto-merge description (the synced workflow only handles `claude/` branches), council README drops its third-party model comparison. Every plugin that lost files gets a patch bump so `/plugin update` propagates the removal: cc-plugin-eval 0.2.3, charon 0.3.1, council 1.3.1, derot 0.2.1, elegance-pipeline 1.2.2, exodia 3.0.2, feature-dev 1.3.1, html-effectiveness 1.2.2, metacognitive-guard 0.7.4, mutation-minded-testing 0.1.1, nihil 0.3.2, nuget-opensrc 0.1.3, tomevault-publish 0.1.1. A second pass scrubbed the remaining living-doc mentions: charon's reviewer examples name Copilot (charon 0.3.2), tomevault-publish's skill drops its cross-CLI tool list (0.1.2), cc-plugin-eval deletes the porting-era SPEC.md/REVIEW.md and condenses its README fork section to an attribution pointing at THIRD_PARTY_NOTICES.md (0.2.4); TOMEVAULT_BADGE.md and the automation bench example are reworded. A third pass rewrote the history files (this changelog, charon's changelog, ADR-0002) the same way, deleted the root second-CLI agent directory (31 agent TOMLs), and scrubbed the last config/script/comment mentions (`.coderabbit.yaml`, `.gitignore`, `.tome/tome.yaml`, automation bench scripts, one stopword, two code comments); the MIT attribution — THIRD_PARTY_NOTICES.md plus the per-file license headers in cc-plugin-eval's source — is the single exception, kept as legally required.
...
- **AGENTS.md single-source / render model (#289, #299)**: The admin-authorized root cleanup (#289) deleted `AGENTS.md` and its one generated copy (the agent-config single source plus its render) along with the entire `tooling/` tree (`weave-validate.sh`, `sync-marketplace.sh`, the plugin template) and several stale `docs/` guides — all unused for ~2 weeks. #299 then retired the now-dead "Edit AGENTS.md only; regenerate via TomeVault" contract that ~6 carriers still pointed at: **CLAUDE.md is now canonical** (the only config carrier present at HEAD, auto-loaded by Claude Code), with `.cursor/` and `.windsurf/` copies hand-maintained. `.github/copilot-instructions.md` remains a separate hand-maintained document.
...
- **`qyl` governing skill (1.1.0)**: Three-layer distribution model (ADR-0002). Encodes qyl
  ...
  invariants. 4 Loom-grounded evals, third-party agent adapter, distributable via `npx skills add`.
...
- **`elegance-pipeline` plugin (1.0.0)**: Multi-agent code-elegance workflow converted from
  an external CLI bundle to native Claude Code plugin. 4 scouts (sonnet, read-only), 2 judges (opus,
...
- **Third-party PR review automation** (since removed): Added a review workflow plus
  prompts/schemas for an external review bot that reviewed pull requests in a read-only
  sandbox, returned structured verdicts, and published formal GitHub reviews.
...
- **README.md**: Removed an accidentally pasted conversion transcript from the GitHub

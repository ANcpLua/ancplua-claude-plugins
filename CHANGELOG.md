...
- **Codex purge (repo-wide)**: the marketplace is Claude Code only — no other CLI mirrors, no non-Claude models. Deleted the Codex CLI marketplace mirror (`.agents/plugins/marketplace.json`), all 13 `plugins/*/.codex-plugin/plugin.json` manifests, all 24 auto-generated `source-command-*` Codex-migration wrapper skills (which Claude Code was loading as real skills and surfacing in installed components), and `.github/codex/` (review prompt + output schema). Docs updated to match: CLAUDE.md (+ `.cursor`/`.windsurf` mirrors) drop the dual-manifest rule, README drops the Codex install section and the stale `codex/`-branch auto-merge description (the synced workflow only handles `claude/` branches), council README drops its Grok comparison. Every plugin that lost files gets a patch bump so `/plugin update` propagates the removal: cc-plugin-eval 0.2.3, charon 0.3.1, council 1.3.1, derot 0.2.1, elegance-pipeline 1.2.2, exodia 3.0.2, feature-dev 1.3.1, html-effectiveness 1.2.2, metacognitive-guard 0.7.4, mutation-minded-testing 0.1.1, nihil 0.3.2, nuget-opensrc 0.1.3, tomevault-publish 0.1.1. A second pass scrubbed the remaining living-doc mentions: charon's reviewer examples (README + skill) name Copilot instead of Codex (charon 0.3.2), tomevault-publish's skill drops its cross-CLI tool list (0.1.2), cc-plugin-eval deletes the porting-era SPEC.md/REVIEW.md and condenses its README fork section to an attribution pointing at THIRD_PARTY_NOTICES.md, which stays as the MIT-required notice (0.2.4); TOMEVAULT_BADGE.md and the automation bench example are reworded. Historical records (this changelog, per-plugin changelogs, ADR-0002) and license attribution are intentionally not rewritten.

...
#
# Hard failures (block merge):  plugin validate, shellcheck, actionlint, JSON syntax, version mismatch
# Soft warnings (informational): markdownlint, orphaned plugins
...
if [ -n "$CLAUDE_CMD" ] && [ -z "${CLAUDECODE:-}" ]; then
  echo "[1/6] Plugin validation (claude CLI)"
  ...
  echo "[1/6] Plugin validation (SKIPPED — nested Claude session)"
else
  echo "[1/6] Plugin validation (SKIPPED — claude CLI not found)"
fi
...
if command -v shellcheck >/dev/null 2>&1; then
  ...
  SHELL_COUNT=$(tr -cd '\0' < "$SHELL_TMPFILE" | wc -c | tr -d '[:space:]')
...
# ── 6. Marketplace / Plugin Version Sync (HARD) + Orphans (SOFT) ─────────────
echo "[6/6] Version sync (marketplace.json ↔ plugin.json)"

MARKETPLACE=".claude-plugin/marketplace.json"
if [ -f "$MARKETPLACE" ]; then
  SYNC_ERRORS=0

  # Check: every plugin dir has an entry in marketplace.json (soft warn)
  if [ -d "plugins" ]; then
    for d in plugins/*/; do
      ...
      if ! jq -e --arg n "$plugin_name" '.plugins[] | select(.name == $n)' "$MARKETPLACE" >/dev/null 2>&1; then
        soft_warn "plugin '$plugin_name' ($d) not in marketplace.json"
      fi
    done
  fi

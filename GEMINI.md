...
You share this repository with Claude. While `CLAUDE.md` defines the *project rules* (architecture, workflows, definitions), `GEMINI.md` defines your *operational strategy* to adhere to those rules effectively.
...
- **Session Context:** If you need to persist complex analysis across turns, you MAY create a temporary file in `.gemini/tmp/` (create if missing), but prefer `write_todos`.

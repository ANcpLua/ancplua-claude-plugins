...
function renderWorkflowGuide(guide) {
  ...
  return [
    ...
    `<p><strong>cc-plugin-eval will run first:</strong> <code>${escapeHtml(startHere.firstCommand)}</code></p>`,
...
function renderInspectHtml(result) {
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>cc-plugin-eval Inspect</title>
  </head>
  <body>
    <main>
      <h1>cc-plugin-eval Inspect: ${escapeHtml(result.target.name)}</h1>
      <p>Components requested: ${escapeHtml((result.componentsRequested || ["all"]).join(", "))}</p>
      <h2>Recommended Next Step</h2>
      ${renderNextAction(result.nextAction)}
      <h2>Findings</h2>
      ${renderOptionalList(items, "No findings recorded for the requested components.")}
    </main>
  </body>
</html>`;
}

function renderImprovementBriefHtml(result) {
  const requiredItems = (result.requiredFixes || []).map((fix) => `${fix.id}: ${fix.message}`);
  const recommendedItems = (result.recommendedFixes || []).map((fix) => `${fix.id}: ${fix.message}`);
...
<html lang="en">
  ...
  <body>
    <main>
      <h1>Improvement Brief: ${escapeHtml(result.target.name)}</h1>
      <p>${escapeHtml(result.summary)}</p>
      <h2>Recommended Next Step</h2>
      ${renderNextAction(result.nextAction)}
      <h2>Required Fixes</h2>
      ${renderOptionalList(requiredItems, "No required fixes outstanding.")}
      <h2>Recommended Fixes</h2>
      ${renderOptionalList(recommendedItems, "No recommended fixes outstanding.")}
      <h2>Suggested Handoff Prompt</h2>
      <p>${escapeHtml(result.suggestedPrompt || "")}</p>
    </main>
  </body>
</html>`;
}
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>cc-plugin-eval Start Here</title>
  </head>
  <body>
    <main>
      <h1>cc-plugin-eval Start Here: ${escapeHtml(result.target.name)}</h1>
...
<html lang="en">
  ...
  <body>
    <main>
      ...
      <h2>Recommended Next Step</h2>
      ${renderNextAction(result.nextAction)}
      <h2>Use From Claude Code Chat</h2>
...
<html lang="en">
  ...
  <body>
    <main>
      ...
      <p>Claude Code version: ${escapeHtml(result.claudeVersion || "unknown")}</p>
  ...
  if (result.kind === "inspect-result") {
    return renderInspectHtml(result);
  }
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>cc-plugin-eval Report</title>
    <style>
      :root {
        color-scheme: light;
        --bg: #f4faf8;
  ...
  <body>
    <main>
      <h1>cc-plugin-eval Report: ${escapeHtml(result.target.name)}</h1>

...
import {
  ...
  createImprovementBriefNextAction,
...
function renderWorkflowGuide(guide) {
  ...
  return [
    ...
    `cc-plugin-eval will run first: ${startHere.firstCommand}`,
    "",
...
function ensureInspectPayload(payload) {
  return {
    ...payload,
    nextAction: payload.nextAction || createInspectNextAction(payload),
  };
}

function ensureImprovementBriefPayload(payload) {
  return {
    ...payload,
    nextAction: payload.nextAction || createImprovementBriefNextAction(payload),
  };
}
...
function renderWorkflowGuidePayload(payload) {
  ...
  return [
    `# cc-plugin-eval Start Here: ${guide.target.name}`,
    "",
    section("At a Glance", [
...
function renderBudgetExplanation(payload) {
  ...
  return [
    ...
    section("Details", [
      ...
      detailsBlock("Use From Claude Code Chat", renderWorkflowGuide(budgetPayload.workflowGuide)),
    ]),
  ].join("\n");
}
...
function renderMeasurementPlanPayload(payload) {
  ...
  return [
    ...
    section("Details", [
      ...
      detailsBlock("Use From Claude Code Chat", renderWorkflowGuide(plan.workflowGuide)),
    ]),
  ].join("\n");
}
...
function renderBenchmarkTemplateInit(payload) {
  ...
  return [
    ...
    section("Details", [
      ...
      detailsBlock("Use From Claude Code Chat", renderWorkflowGuide(benchmarkPayload.workflowGuide)),
    ]),
  ].join("\n");
}
...
function renderBenchmarkRun(payload) {
  ...
  return [
    ...
    section("At a Glance", [
      ...
      `- Claude Code version: ${benchmarkPayload.claudeVersion}`,
    ...
    "",
    section("Why It Matters", [
      "- Benchmarking now runs real `claude` sessions in isolated workspaces instead of simulating the skill or plugin through a single API request.",
    ...
    section("Details", [
      detailsBlock(
        ...
        benchmarkPayload.scenarios
          .map((scenario) => renderBenchmarkScenario(scenario))
          .join("\n\n"),
      ),
...
function renderComparison(payload) {
  ...
  return [
    `# cc-plugin-eval Comparison: ${diff.target.name}`,
    "",
    section("At a Glance", [
      `- Score delta: ${diff.scoreDelta >= 0 ? "+" : ""}${diff.scoreDelta}`,
...
function renderInspect(payload) {
  const inspectPayload = ensureInspectPayload(payload);
  ...
  for (const finding of inspectPayload.findings || []) {
    ...
    findingsByComponent.get(component).push(finding);
  }

  const sections = [...findingsByComponent.entries()].map(([component, findings]) => {
    const lines = findings.map((finding) => {
      ...
      return `- [${finding.severity}/${finding.code}] ${finding.message}${where}${fix}`;
    });
    return [`## ${component}`, ...lines].join("\n");
  });

  return [
    `# cc-plugin-eval Inspect: ${inspectPayload.target.name}`,
    "",
    section("At a Glance", [
      `- Components requested: ${(inspectPayload.componentsRequested || ["all"]).join(", ")}`,
      `- Findings: ${inspectPayload.summary?.findingCounts?.total || 0} (${inspectPayload.summary?.findingCounts?.error || 0} error, ${inspectPayload.summary?.findingCounts?.warn || 0} warn, ${inspectPayload.summary?.findingCounts?.info || 0} info)`,
      `- Score: ${inspectPayload.summary?.score || 100}/100`,
    ]),
    "",
    section("Recommended Next Step", renderNextAction(inspectPayload.nextAction)),
    "",
    sections.length > 0 ? sections.join("\n\n") : "## Findings\n\n- No findings recorded for the requested components.",
  ].join("\n");
}

function renderImprovementBrief(payload) {
  const briefPayload = ensureImprovementBriefPayload(payload);

  return [
    `# Improvement Brief: ${briefPayload.target.name}`,
    "",
    section("At a Glance", [
      `- Title: ${briefPayload.title}`,
      `- Required fixes: ${briefPayload.requiredFixes?.length || 0}`,
      `- Recommended fixes: ${briefPayload.recommendedFixes?.length || 0}`,
      `- Measurement goals: ${briefPayload.measurementGoals?.length || 0}`,
    ]),
    "",
    section("Why It Matters", [`- ${briefPayload.summary}`]),
    "",
    section(
      "Fix First",
      briefPayload.requiredFixes?.length > 0
        ? briefPayload.requiredFixes.map((fix) => `- ${fix.id}: ${fix.message}`)
        : ["- No required fixes outstanding."],
    ),
    "",
    section("Recommended Next Step", renderNextAction(briefPayload.nextAction)),
    "",
    section("Details", [
      detailsBlock(
        "Recommended fixes",
        briefPayload.recommendedFixes?.length > 0
          ? briefPayload.recommendedFixes.map((fix) => `- ${fix.id}: ${fix.message}`).join("\n")
          : "- None",
      ),
      detailsBlock(
        "Suggested handoff prompt",
        briefPayload.suggestedPrompt || "- None",
      ),
    ]),
  ].join("\n");
}
...
export function renderMarkdown(payload) {
  ...
  if (payload.kind === "inspect-result") {
    return renderInspect(payload);
  }
  if (payload.kind === "improvement-brief") {
    return renderImprovementBrief(payload);
  }

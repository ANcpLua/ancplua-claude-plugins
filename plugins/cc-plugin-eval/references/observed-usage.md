...
`cc-plugin-eval` can ingest local JSON or JSONL files that contain token telemetry from real runs.

## Supported Shapes

The parser is provider-agnostic and accepts:

- a Claude Messages API or Responses API object with a top-level `usage`
...
```bash
cc-plugin-eval analyze ./skills/my-skill --observed-usage ./runs/responses.jsonl
cc-plugin-eval measurement-plan ./skills/my-skill --observed-usage ./runs/responses.jsonl --format markdown
```

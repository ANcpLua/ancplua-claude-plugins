...
def load_rules(event: Optional[str] = None) -> List[Rule]:
    """Load all hookify rules from .claude directory and global rules.
    ...
    1. ~/.claude/global-rules/hookify.*.local.md (global rules, apply everywhere)
    2. .claude/hookify.*.local.md (project-specific rules)
    ...
    for file_path in files:
        try:
            ...
            # Skip if we've already seen a rule with this name (local takes precedence)

...
The skill expects Thariq's repo cloned at `~/RiderProjects/thariq-html-effectiveness/` so it can study the canonical source byte-for-byte. If missing, the skill instructs Claude to run `gh repo clone ThariqS/html-effectiveness ~/RiderProjects/thariq-html-effectiveness`.

...
export function registerTools(server: McpServer, store: EventStore): void {
  server.registerTool(
    "get_status",
    {
  ...
  server.registerTool(
    ...
    async ({ session_id }) => {
      ...
      const summary = events.map((e) => ({
        ...
        tool: e.tool_name,
        agent: e.agent_name,
        file: (e.tool_input as Record<string, unknown>)?.file_path,
        command: (e.tool_input as Record<string, unknown>)?.command,
      }));
      return {
        content: [
          {

---
name: source-command-exodia-deep-think
description: 'IF complex problem needing multi-perspective analysis THEN use this. 5 agents, 3
  phases: understand->synthesize->recommend. No implementation. For competing implementations
  -> tournament.'
---

# source-command-exodia-deep-think

Use this skill when the user asks to run the migrated Claude slash command `/exodia:deep-think`.
...
Migrated from Claude slash command `/exodia:deep-think` into a Codex skill. Invoke it as `$source-command-exodia-deep-think` and rewrite provider-specific runtime behavior before relying on it for unattended execution.

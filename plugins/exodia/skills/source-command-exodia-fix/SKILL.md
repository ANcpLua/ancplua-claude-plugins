---
name: source-command-exodia-fix
...
Use this skill when the user asks to run the migrated Claude slash command `/exodia:fix`.
...
Migrated from Claude slash command `/exodia:fix` into a Codex skill. Invoke it as `$source-command-exodia-fix` and rewrite provider-specific runtime behavior before relying on it for unattended execution.

---
name: source-command-exodia-tournament
description: IF multiple valid approaches THEN use this. N agents compete, judge picks winner by
  penalty scoring. For trade-offs without competition -> deep-think.
---

# source-command-exodia-tournament

Use this skill when the user asks to run the migrated Claude slash command `/exodia:tournament`.
...
Migrated from Claude slash command `/exodia:tournament` into a Codex skill. Invoke it as `$source-command-exodia-tournament` and rewrite provider-specific runtime behavior before relying on it for unattended execution.

---
description: "IF a bug needs fixing THEN use this — the one exodia fix command. 4 gated phases (analyze → design → implement → verify). Args: severity P0|P1|P2|P3 and parallelism standard|maximum (for a P0 emergency pass `P0 maximum`). Auto-inherits `.eight-gates/artifacts/findings.json` from a prior mega-swarm run when present."

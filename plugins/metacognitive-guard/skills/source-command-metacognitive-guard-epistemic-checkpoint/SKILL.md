---
name: source-command-metacognitive-guard-epistemic-checkpoint
description: Force verification before answering questions involving versions, dates, status, or
  "current" state. Prevents hallucinations at the REASONING level by checking assertions.yaml
  and WebSearch before forming beliefs.
---

# source-command-metacognitive-guard-epistemic-checkpoint

Use this skill when the user asks to run the migrated Claude slash command `/metacognitive-guard:epistemic-checkpoint`.
...
Migrated from Claude slash command `/metacognitive-guard:epistemic-checkpoint` into a Codex skill. Invoke it as `$source-command-metacognitive-guard-epistemic-checkpoint` and rewrite provider-specific runtime behavior before relying on it for unattended execution.

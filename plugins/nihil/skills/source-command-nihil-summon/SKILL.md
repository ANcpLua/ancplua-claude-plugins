---
name: source-command-nihil-summon
description: Install Nihil's dynamic workflows into this project's .claude/workflows so they run
  as /nihil, /nihil-maat, /nihil-odin, /nihil-shiva, /nihil-athena
---

# source-command-nihil-summon

Use this skill when the user asks to run the migrated Claude slash command `/nihil:summon`.
...
Migrated from Claude slash command `/nihil:summon` into a Codex skill. Invoke it as `$source-command-nihil-summon` and rewrite provider-specific runtime behavior before relying on it for unattended execution.

---
name: source-command-charon-charon
description: "Ferry GitHub PR(s) to merge \u2014 an honest snapshot-not-block loop that fixes CI,\
  \ repairs conflicts, version-checks reviewer suggestions, and never says just wait.\
  \ One PR (or current branch) = solo ferry with a resume net; several PRs = a ferryman-led\
  \ agent team; 'cancel' stops it."
---

# source-command-charon-charon

Use this skill when the user asks to run the migrated Claude slash command `/charon:charon`.
...
Migrated from Claude slash command `/charon:charon` into a Codex skill. Invoke it as `$source-command-charon-charon` and rewrite provider-specific runtime behavior before relying on it for unattended execution.
...
Review unsupported Claude slash-command metadata manually: `allowed-tools`, `argument-hint`.
